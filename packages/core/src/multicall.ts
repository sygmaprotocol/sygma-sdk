...
export async function getFeeHandlerAddressesOfRoutes(
    params: {
...
export async function getFeeHandlerTypeOfRoutes(
    params: {
...
> {
    ...
    for (let i = 0; i < params.routes.length; i++) {
        calls.push({
            ...
            callData: basicFeeHandlerInterface.encodeFunctionData(
                'feeHandlerType'
            )
        })
    }
